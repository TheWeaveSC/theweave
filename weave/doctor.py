...
from .lease import CONFLICT_COPY_RE
...
def _detect_layout(vault: Vault) -> tuple[str, dict[str, list[str]]]:
    ...
    sometimes have multiple (e.g. both vault/sessions/ and wiki/sessions/) —
    reporting all of them is more honest than first-match-wins.
    """
...
def check_vault(vault_path: str | None) -> CheckGroup:
    ...
    # iCloud conflict-copy detector (TN2336-style "name 2.md" siblings) —
    # purely detective, never fails the run.
    #
    # HEURISTIC WARN, not authoritative: this regex (shared with
    # weave/lease.py's active pre-write guard, defined once as
    # CONFLICT_COPY_RE) matches ANY filename ending in a space + digits +
    # ".md", including legitimately-named notes such as "weekly-2026 3.md"
    # (a real note the user named that way, not an iCloud conflict copy).
    # A match here is a prompt for a human to glance at the file, not proof
    # of an actual sync conflict — a future reader should not treat this
    # WARN as ground truth without opening the file(s) in question.
    conflicts: list[str] = []
    for path in vault.root.rglob("*.md"):
        if any(p.startswith(".") for p in path.relative_to(vault.root).parts):
            continue
        if CONFLICT_COPY_RE.search(path.name):
            conflicts.append(str(path.relative_to(vault.root)))
    if conflicts:
        sample = ", ".join(conflicts[:3])
        more = f" ... and {len(conflicts) - 3} more" if len(conflicts) > 3 else ""
        g.add("icloud-conflicts", WARN,
              f"{len(conflicts)} iCloud conflict-copy file(s) detected",
              detail=sample + more)
    else:
        g.add("icloud-conflicts", PASS, "No iCloud conflict-copy files detected")

    # .trash/ tombstone health — informational only.
    trash_dir = vault.root / ".trash"
    if trash_dir.is_dir():
        trashed = [p for p in trash_dir.glob("*.md")]
        g.add("trash", INFO, f".trash/ holds {len(trashed)} tombstoned note(s)")
    else:
        g.add("trash", INFO, ".trash/ does not exist yet (no deletes performed)")
