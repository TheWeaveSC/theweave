...
# ---------- team (Org Team Engine) ----------

from .team.cli import team as _team_group  # noqa: E402
...
def demo_fixture(out: str) -> None:
    ...
    Path(out).write_text(
        ...
        "date: 2026-05-23\n"
        "type: session\n"
        'touches: ["[[entity-ACME-v1.2.8]]", "[[entity-Eddie]]"]\n'
        "---\n\n"
        "# ACME Dry-Run Green\n\n"
        "Re-tested replication after WAL tuning. Lag well under threshold.\n"
        "Eddie approved. Migration window confirmed for 2026-06-01.\n",
        encoding="utf-8",
    )
...
# ---------- init (L4 one-click installer) ----------

@cli.command("init")
@click.option("--vault", "-V", required=True, help="Path to scaffold the new vault at.")
@click.option("--target", type=click.Choice(["auto", "desktop", "code", "both", "none"]),
              default="auto", show_default=True, help="Which Claude config(s) to wire.")
@click.option("--claude-desktop-config", default=None,
              help="Override path for claude_desktop_config.json (test/automation seam).")
@click.option("--claude-code-config", default=None,
              help="Override path for ~/.claude.json (test/automation seam).")
@click.option("--python", "python_path", default=None,
              help="Interpreter to write into the MCP command. Defaults to sys.executable.")
@click.option("--yes", "-y", is_flag=True, default=False, help="Skip the confirm prompt.")
@click.option("--dry-run", is_flag=True, default=False, help="Print the plan; write nothing.")
@click.option("--force", is_flag=True, default=False,
              help="Allow overwriting existing boot/team files (tombstoned).")
def init_cmd(vault: str, target: str, claude_desktop_config: str | None,
             claude_code_config: str | None, python_path: str | None,
             yes: bool, dry_run: bool, force: bool) -> None:
    """One-click installer — scaffold a vault, install the team, wire Claude's MCP config."""
    from .installer import InstallOptions, run_init
    opts = InstallOptions(
        vault=vault,
        target=target,
