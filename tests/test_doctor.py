...
# ---------- iCloud conflict-copy + trash detective checks ----------

def test_icloud_conflict_copy_detected(tmp_path: Path) -> None:
    _write_canonical_vault(tmp_path)
    (tmp_path / "FooVault" / "entities" / "entity-Foo 2.md").write_text(ENTITY_BODY)
    g = check_vault(str(tmp_path))
    c = next(chk for chk in g.checks if chk.name == "icloud-conflicts")
    assert c.status == WARN
    assert "entity-Foo 2.md" in (c.detail or "")


def test_icloud_conflict_copy_not_triggered_by_normal_filename(tmp_path: Path) -> None:
    _write_canonical_vault(tmp_path)
    (tmp_path / "FooVault" / "entities" / "entity-Foo2.md").write_text(ENTITY_BODY)
    g = check_vault(str(tmp_path))
    c = next(chk for chk in g.checks if chk.name == "icloud-conflicts")
    assert c.status == PASS


def test_trash_check_info_when_no_trash_dir(tmp_path: Path) -> None:
    _write_canonical_vault(tmp_path)
    g = check_vault(str(tmp_path))
    c = next(chk for chk in g.checks if chk.name == "trash")
    assert c.status == INFO
    assert "does not exist" in c.message


def test_trash_check_counts_tombstoned_notes(tmp_path: Path) -> None:
    from weave.vault import Vault as _Vault
    _write_canonical_vault(tmp_path)
    v = _Vault(tmp_path)
    v.write_text("scratch.md", "x")
    v.delete("scratch.md")
    g = check_vault(str(tmp_path))
    c = next(chk for chk in g.checks if chk.name == "trash")
    assert c.status == INFO
    assert "1 tombstoned" in c.message


def test_doctor_warnings_never_fail_exit_code(tmp_path: Path) -> None:
    _write_canonical_vault(tmp_path)
    (tmp_path / "FooVault" / "entities" / "entity-Foo 2.md").write_text(ENTITY_BODY)
    report = run_doctor(str(tmp_path), include_mcp=False)
    assert report.failures() == 0
